...
MAX_SPEED = 200.0 / 3.6  # maximum speed [m/s]
...
def calc_frenet_paths(c_s_d, c_s_dd, c_d, c_d_d, c_d_dd, s0):
    ...
    for Ti in np.arange(MIN_T, MAX_T, DT):
        lon_paths = VelocityKeepingLongitudinalMovementStrategy().calc_longitudinal_trajectory(
            c_s_d, c_s_dd, Ti, s0
        )

        for fp in lon_paths:
            for di in VelocityKeepingLongitudinalMovementStrategy().get_d_arrange(s0):
                tp = HighSpeedLateralMovementStrategy().calc_lateral_trajectory(
                ...
                lon_cost = (
                    K_J * Js
                    + K_T * Ti
                    + VelocityKeepingLongitudinalMovementStrategy().calc_destination_cost(tp)
                )
...
def calc_global_paths(fplist, csp):
    return [
        HighSpeedLateralMovementStrategy().calc_cartesian_parameters(fp, csp) for fp in fplist
    ]
